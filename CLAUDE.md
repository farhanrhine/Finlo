...
├── database/
│   ├── __init__.py
│   └── db.py           # SQLite helpers and SQLAlchemy ORM operations
├── templates/
...
- DB queries: use SQLAlchemy ORM methods, parameterized queries with `?` placeholders for raw SQL
...
- **Database**: SQLite with SQLAlchemy ORM — no PostgreSQL, no external databases
...
- **Never commit without a meaningful message** — use conventional commit style
